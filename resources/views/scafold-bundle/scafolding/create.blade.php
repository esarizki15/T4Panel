                                ...
                                @forelse($columns as  $column)
                                    ...
                                    <div class="form-group row @if( $column->type == "hidden" ) {{ 'hide' }}  @endif  ">
                                          @if( $column->type == "hidden" )
                                                @else
                                        <label for="name" class="col-md-2 col-form-label text-md-right">{{   strtoupper(implode(' ',explode('_',  $column->label )))  }}</label>
                                                    @endif
                                            ...
                                            <div class="col-md-11">
                                                                ...
                                                                <div class="col-lg-6 col-md-6">
                                                                    <div class="input-group">
                                                                        <span class="input-group-addon">
                                                                            <image src="{{$collection->image}}"  style="width:20px;" />
                                                                        </span>
                                                                        <input id="{{ $column->name }}{{ $kcoll }}image" type="hidden" 
                                                                        class="form-control" 
                                                                        name="{{ $column->name }}[{{ $kcoll }}][image]"  
                                                                        value="{{ $collection->image }}"  /> 
                                                                        <input id="{{ $column->name }}{{ $kcoll }}color" type="hidden" 
                                                                        class="form-control" 
                                                                        name="{{ $column->name }}[{{ $kcoll }}][color]"  
                                                                        value="{{ $collection->color }}"  /> 
                                                                        <input id="{{ $column->name }}{{ $kcoll }}text" type="text" 
                                                                        class="form-control" 
                                                ...
                                                @else
                                                    ...
                                                    @if($column->type == "textarea" )
                                                        <textarea  id="{{ $column->name }}" type="{{ $column->type }}" class="form-control   " name="{{ $column->name }}"   @if($column->not_required=='NOT') {{ 'required' }} @endif @if($column->disabled=='disabled') {{ 'disabled' }} @endif  >{{ old($column->name) }}</textarea> 
                                                    @endif
                                                    @if($column->type == "textarea-wysihtml5" )
                                                        <textarea  id="{{ $column->name }}" type="{{ $column->type }}" class="form-control wysihtml5-editor  " name="{{ $column->name }}"   @if($column->not_required=='NOT') {{ 'required' }} @endif @if($column->disabled=='disabled') {{ 'disabled' }} @endif  >{{ old($column->name) }}</textarea> 
                                                    @endif
