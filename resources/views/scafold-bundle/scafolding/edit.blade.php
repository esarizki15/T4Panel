                           ...
                           @forelse(  json_decode($model)  as  $key => $val )
                                        @if ( $key != 'id'  ) 
                                            <div class="form-group row @if( isset($columns[$key]) && $columns[$key]->type == "hidden" ) {{ 'hide' }} @endif ">
                                                ...
                                                @else
                                                <label for="name" class="col-md-2 col-form-label text-md-right">{{    strtoupper(implode(' ',explode('_',( isset( $columns[$key]) && isset($columns[$key]->label)  ? $columns[$key]->label : $key ))))  }}</label>
                                                @endif
                                                    ...
                                                    <div class="col-md-10">
                                                        @if( isset($columns[$key]) && $columns[$key]->data  ) 
                                                            @if( $columns[$key]->type=='multiple-field' )
                                                                    ...
                                                                    <div class="form-group row">
                                                                        <div class="col-lg-6 col-md-6">
                                                                            <div class="input-group">
                                                                                <span class="input-group-addon"><image src="{{$collection->image}}"  style="width:20px;" /></span>
                                                                                <input id="{{ $columns[$key]->name }}{{ $kcoll }}image" type="hidden" 
                                                                                class="form-control" 
                                                                                name="{{ $columns[$key]->name }}[{{ $kcoll }}][image]"  
                                                                                value="{{ $collection->image }}"  />
                                                                                <input id="{{ $columns[$key]->name }}{{ $kcoll }}color" type="hidden" 
                                                                                class="form-control" 
                                                                                ...
                                                                                @if(is_array( json_decode($val)))
                                                                                    @if( isset(json_decode($val)[$kcoll]) && isset(json_decode($val)[$kcoll]->text) )
                                                                                        <input id="{{ $columns[$key]->name }}{{ $kcoll }}text" type="text" 
                                                                                        class="form-control" 
                                                                ...
                                                                @endforelse
                                                            @else
                                                                <select class="select2  @if($columns[$key]->type=='multiple-select') {{ 'col-lg-6' }} @else {{ 'col-lg-4' }} @endif " 
                                                        ...
                                                        @else
                                                            ...
                                                            @if($columns[$key]->type == "textarea" )
                                                                <textarea  id="{{ $key }}" type="{{ $columns[$key]->type }}" class="form-control   " name="{{ $key }}"   @if($columns[$key]->not_required=='NOT') {{ 'required' }} @endif @if($columns[$key]->disabled=='disabled') {{ 'disabled' }} @endif  >{{ $val }}</textarea> 
                                                            @endif
                                                              @if($columns[$key]->type == "textarea-wysihtml5" )
                                                                <textarea  id="{{ $key }}" type="{{ $columns[$key]->type }}" class="form-control wysihtml5-editor    " name="{{ $key }}"   @if($columns[$key]->not_required=='NOT') {{ 'required' }} @endif @if($columns[$key]->disabled=='disabled') {{ 'disabled' }} @endif  >{{ $val }}</textarea> 
                                                            @endif
