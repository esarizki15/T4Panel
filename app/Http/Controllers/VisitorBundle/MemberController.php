...
class MemberController extends ScafoldController
{
    ...
    public function __construct(){
        ...
        $this->setCaption(\App\Http\Model\SubMenu::where("code","{$this->moduleName}-member")->first());
    }
    ...
    {   
        ...
        $model->where('appointment_member.client_type','=',$this->clientType);
        $assetsPath = $this->getUrlPath();
        $caption = $this->getCaption();
        $columns = json_decode(json_encode([ 
            ...
            [ "name" => "customers" , "type" => "character varying" , "table" => "" , "where" => "member.member_nm" ],
            ...
            [ "name" => "plat_number" , "type" => "character varying" , "table" => "" , "where" => "appointment_member.identity_code" ],
    ...
    public function show($id){
        ...
        $model = $model[0];
        return  $this->view('vaccine-bundle.vaccine-member.show',compact('model', "caption" ,'assetsPath' ));
    }
        ...
        if($model){
            if(in_array($model['status'], [1,6])){
                  try{
                    ...
                    $member_model = Member::select("*")->find($member_id);
                    $push_model = PushNotification::insert([
                        "member_id" => $member_id,
                        "push_notification_text" => "Pembayaran anda telah di verifikasi",
                        "push_notification_title" => "Vaccine & Swab",
                        "push_notification_url" => "",
                        "push_notification_url_host" => "",
                        "push_notification_token" => $member_model->member_token_push,
