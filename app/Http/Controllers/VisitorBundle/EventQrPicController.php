...
class EventQrPicController extends ScafoldController
{
    ...
    public function __construct(){
        ...
        $collection['event_at']= json_decode( json_encode([ 
            ...
            ["id"=>"{$this->clientType}", "name"=> "{$this->clientTypelabel}" ] , 
            // ["id"=>"RESIDENT_SERVICE", "name"=> "RESIDENT SERVICE" ] , 
        ]) );
        $appointmentLocation =  AppointmentServiceLocationModel::select("id","location_name")->where("client_type",$this->clientType)->get();
        ...
        $this->setCaption(\App\Http\Model\SubMenu::where("code","{$this->moduleName}-event-qr-pic")->first());
    }
    ...
    {   
        ...
        $model->whereIn('event_qr_pic.event_at',[$this->clientType]);
        $assetsPath = $this->getUrlPath();
